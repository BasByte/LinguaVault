...
export default async function ManagerCourseEditPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [course, lessons, languages] = await Promise.all([
    ...
    prisma.lessons.findMany({ where: { course_id: courseId }, orderBy: { order_index: "asc" } }),
