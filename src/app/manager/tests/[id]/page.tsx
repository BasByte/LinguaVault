...
export default async function ManagerTestEditPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [test, questions, languages] = await Promise.all([
    prisma.tests.findUnique({ where: { id: testId }, include: { languages: { select: { name: true, flag_emoji: true } } } }),
    prisma.test_questions.findMany({ where: { test_id: testId }, orderBy: { order_index: "asc" } }),
  ...
  const testForEditor = {
    ...
    cefr_level: test.cefr_level, language_id: test.language_id ?? 0,
    duration_minutes: test.duration_minutes, passing_score: test.passing_score,
    is_published: test.is_published, is_public: test.is_public,
  ...
  const questionsForEditor = questions.map((q) => ({
    ...
    options: q.options as string[] | null, correct_answer: q.correct_answer,
